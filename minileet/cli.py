...
def _eval(args: argparse.Namespace) -> None:
    ...
    for task in tasks:
        attempt = _solve_task(args.baseline, task, args.budget, args.hidden_count, args.seed)
        if attempt is None:
...
def _solve_task(
    ...
    if baseline == "enumerative":
        return solve_with_enumeration(task, budget=budget, hidden_count=hidden_count, seed=seed)
    if baseline == "random":
        return solve_with_random_search(task, budget=budget, hidden_count=hidden_count, seed=seed)
